...
main = interact ((read::String->Sudoku)
                   >>> makeGame
                   -- >>> trim row
                   -- >>> trim column
                   -- >>> trim box
...
instance (Game m, GSpace m ~ sq, sq ~ GSpace (RemovePlayed sq m))
   ...
   options s = do
       m <- get
       if s `elem` m then mzero else raise pieces
   updates [] = return Continue
   updates ((sq,pc):xs) = do
       o <- options sq
       if pc `elem` o then mzero
       else do  modify $ insertSet sq
                g <- get
                s <- raise spaces
...
instance (Game m, GSpace m ~ sq, GPiece m ~ v, View sq x, Ord x)
    ...
    updates xs = 
        fmap concat $ flip mapM xs $ \(sq,v) -> do
             ...
             if  v `elem` m then do
                 modify $ insertWith (difference) (view sq) (setFromList [v])
                 return Continue
             else mzero
    options x = do
          opts <- options x
